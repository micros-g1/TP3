...
void fsk_tx_init(fsk_tx_next_bit_callback callback)
{
	static bool initialized = false;
		...
		pit_conf_t pit_conf1 = {.callback=next_bit_request_handler, .chain_mode=false, .channel=PIT_CH2, .timer_count=BIT_TIME_COUNTER_VALUE, .timer_enable=true, .timer_interrupt_enable=false};
...
{
	pit_set_timer_interrupt_enabled(PIT_CH2, ie);
}
