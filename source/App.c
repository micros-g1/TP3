...
void App_Init (void){
	...
	dma_conf_t conf2 = {.citer=1, .destination_address=0x400CC000, .destination_address_adjustment=0, .destination_data_transfer_size=DMA_16BIT,
						.destination_offset=0, .dma_mux_conf=mux_conf2, .nbytes=sizeof(uint16_t), .source_address=0x400BB012, .source_address_adjustment=0,
						.source_data_transfer_size=DMA_16BIT, .source_offset=0};
	dma_set_config_channel(conf2);

	/* pit */
	pit_init();
	pit_conf_t pit_conf = {.callback=value_togle, .chain_mode=false, .channel=PIT_CH0, .timer_count=0xEE6B280, .timer_enable=true, .timer_interrupt_enable=true};
